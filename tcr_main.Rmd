...
sc.dir <- "/Users/rpanchal/Desktop/tcrquant"
fl.dir <- "/Users/rpanchal/Desktop/tcrquant"
er.file <- "/Users/rpanchal/Desktop/tcrquant/log.txt"
er.msg <- "Early Exit code. Please check error log."
cwd <- "/data2"
fil <- "Yes"
files.A <- list.files(paste(fl.dir,cwd,sep = ""),pattern = "A.*txt")
files.B <- list.files(paste(fl.dir,cwd,sep = ""),pattern = "B.*txt")
...
immdataA <- parse.file.list(paste(fl.dir,cwd,"/",files.A,sep = ""), .format = 'mixcr')
immdataB <- parse.file.list(paste(fl.dir,cwd,"/",files.B,sep = ""), .format = 'mixcr')
...
gdiv <- ggplot(data = div.df, aes(y = div, x = row.names(div.df), fill = group)) + geom_bar(stat = "identity") + xlab("Samples") + ylab("Diversity") + ggtitle("True Diversity Index") + 
...
div.df$group <- c(rep("A",numA), rep("B",numB))
#div.df$group <- c(rep("Control",2), rep("Vaccine",2),rep("Vaccine+PDL1",2))
gsimp <- ggplot(data = div.df, aes(y = div, x = row.names(div.df), fill = group)) + geom_bar(stat = "identity") + xlab("Samples") + ylab("Diversity") + ggtitle("Inverse Simpson Index") + 
theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), panel.background = element_blank(), axis.line = element_line())
print(gsimp)
```
